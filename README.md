...
Планы
Дальнейшие идеи развития проекта:

Поддержка нескольких LLM‑провайдеров

Добавить режимы yandex, gigachat, mock в LLM_PROVIDER:

yandex — интеграция с YandexGPT API (через Yandex Cloud, Python SDK или REST).[web:155][web:156][web:161]

gigachat — интеграция с GigaChat API от Сбера для работы полностью внутри РФ.[web:124][web:162]

mock — локальный «заглушечный» режим без внешнего API (для отладки и offline‑демо).

Улучшение RAG‑логики

Нормализация/очистка текста FAQ (markdown, списки, заголовки).

Больше метаданных в индекс (категория вопроса, тэги).

Поддержка нескольких источников знаний (FAQ + условия доставки + тарифы).

Продвинутый Telegram‑интерфейс

Inline‑кнопка «Открыть оригинал на сайте» с URL из FAQ.

Кнопка «Не помогло» → логирование фидбэка и пометка диалога.

Простая статистика для админа: количество запросов, топ‑вопросы.

Деплой на сервер

Подготовка Dockerfile и docker-compose.yml.

Деплой на VPS (например, Hetzner / BestServer / Timeweb Cloud) или PaaS:

systemd‑сервис или docker‑контейнер.

Перенос .env в переменные окружения сервера.

Возможный деплой на российский хостинг + LLM‑провайдер внутри РФ (YandexGPT/GigaChat), чтобы всё работало без VPN.[web:160][web:163]

Админ‑панель / просмотр логов

Простой web‑интерфейс (FastAPI + шаблоны) для просмотра dialog_logs.

Фильтрация по дате, юзеру, ключевым словам.

Экспорт логов в CSV/Excel для анализа.

Если будешь добавлять что‑то из раздела «Планы», можно будет обновить README, пометив реализованные пункты галочками.
